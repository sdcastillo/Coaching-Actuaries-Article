...
As the graphs below indicate, as more time was spent practicing, my earned level increased for each exam.  For exam P, my earned level was highest at about 7.3.  For MFE, the earned level was less significant as a larger majority of my practice time was spent on quizzes, which do not impact earned level.
...
numeric_cols = adapt_data[,numeric_index] %>%
  ...
  as.matrix() 
corrplot(cor(numeric_cols),method = "number", type = "upper", tl.col = "black")
```
...
Variable imoportance gives a measure of how the value of a given variable influences the outcome of `correct`, when holding all other variables constant.  This plot below was generated using a random forest model, and shows which factors are most important to whether a question is answered correctly or not. 
...
grid.arrange(make_rf_partial_plot(rf2, "difficulty", "Question Difficulty"),
             ...
             make_rf_partial_plot(rf2, "hist_net_diff", "Sum of Difficulty for Previous Easier Questions")
                     )
```
```{r}
diff_minutes_used_contour <- rf2 %>% 
  pdp::partial(pred.var = c("difficulty", "minutes_used")) %>% 
  autoplot(rug = T, train = training, contour = T)

diff_minutes_used_contour 

pdp::partial(rf2,pred.var = c("difficulty"), train = training)

iris_rf <- randomForest(Species~., data = iris, probability = TRUE)
iris_rf

par.Petal_W.Sepal_W <- pdp::partial(iris_rf, pred.var = c("Petal.Width", "Sepal.Width"), chull = TRUE)
...
- Actuarial exam practice questions on the Adapt system can be predicted.  The exam-takers history was measured to be meaningful in predicting future exam performance and 14 historical features were evaluated for predictive power.  The type of model used is far less significant than the quality of the input features used.  
